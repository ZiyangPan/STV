SoftThreshVarying <- function(z,y,w,rho=NULL,n_folds=5,w_grid=NULL){
  ...
  gma_alpha = ini_gma(z,y,w,q_n = q_n)
  gma_ini = gma_alpha$gma_ini
  alpha = gma_alpha$alpha
  STV_fit = optim(par = gma_ini, llk_linear_penal_Rcpp, z=z,y=y,q_n=q_n,B = B,alpha= alpha,rho=rho, method ="BFGS",control = list(maxit=5000) ) 
  STV_llk = STV_fit$value
  STV_gma = matrix(STV_fit$par,nrow = q_n, nc = p)

  STV_conv = STV_fit$convergence
  ...
  return(list(beta = STV_beta_grid, sds = STV_sd_grid, q_n = q_n, alpha = alpha, theta = STV_theta_grid, conv = STV_conv))
}

### lines 36 - 1325 are the same as lines 31 - 1320 in STV_linear_functions.R
### some comments and minor modifications are made for function STV_linear_simulation
...
STV_linear_cv <- function(z,y,w,rho, n_folds = 5, llk_fun){
  ...
  for(fold in 1:n_folds){
    ...
    for(l in 1:L){
      ...
      B_train = B[-test_i,]
      B_test = B[test_i,]
      STV_fit = optim(par = gma_ini, llk_fun, z=z_train,y=y_train,q_n=h[l],B = B_train,alpha= alpha,rho=rho, method ="BFGS",control = list(maxit=5000) )
      gma =  matrix(STV_fit$par,nrow = q_n, nc = p)
      theta = B_test%*%gma
      beta = stv_beta_t_thre(gma,q_n=q_n,alpha,B = B_test)
      y_hat = rowSums(sapply(1:p,function(x) z_test[,x]*beta[,x]))
...
STV_linear_simulation = function(n, p, max_time = 3, err_sd_ratio = 0.2, f_list, f_pos, seed = 12, cov_type = "AR1",z_rho = 0.5,sigma=1){
  ...
  if(cov_type == "AR1"){
    ...
    z_mat = matrix(z_rho, p,p) # for covariance type: independent, z_rho = 0, sigma = 1; for covariance type: compound symmetry, sigma = 1
...
btsp_ds = function(dataset, n_boot) {
  n_row = nrow(dataset)
  res = data.frame(matrix(ncol = 4, nrow = 0))
  for (k in 1:n_boot) {
    set.seed(k)
    sample_idx = sample(x = 1:n_row, size = n_row, replace = TRUE)
    temp_data = dataset[sample_idx, ]
    temp_res = cbind.data.frame(temp_data, k_idx = k)
    res = rbind.data.frame(res, temp_res)
  }
  
  return(res)
}

btsp_ls = function(list, n_boot) {
  n_row = length(list)
  res = data.frame(matrix(ncol = 2, nrow = 0))
  for(k in 1:n_boot) {
    set.seed(k)
    sample_idx = sample(x = 1:n_row, size = n_row, replace = TRUE)
    temp_data = list[sample_idx]
    temp_res = cbind.data.frame(temp_data, k_idx = k)
    res = rbind.data.frame(res, temp_res)
  }
...
get_btsp_res = function(sim_data, n_btsp) {
  z = sim_data$z
  y = sim_data$y
  w = sim_data$w
  
  z_all_btsp = btsp_ds(dataset = z, n_boot = n_btsp)
  y_all_btsp = btsp_ls(list = y, n_boot = n_btsp)
  w_all_btsp = btsp_ls(list = w, n_boot = n_btsp)
  
  result1_work = matrix(ncol = 3, nrow = 0)
  result2_work = matrix(ncol = 3, nrow = 0)
  result3_work = matrix(ncol = 3, nrow = 0)
  
  res_conv_work = c()
  
  for (l in 1:n_btsp) {
    z_work = data.matrix(z_all_btsp[which(z_all_btsp$k_idx == l),c(1:3)])
    y_work = y_all_btsp[which(y_all_btsp$k_idx == l),1]
    w_work = w_all_btsp[which(w_all_btsp$k_idx == l),1]
    
    z_work = z_work[order(w_work),]
    y_work = y_work[order(w_work)]
    w_work = w_work[order(w_work)]
    
    
    stv.fit = SoftThreshVarying(z_work, y_work, w_work, rho=NULL, n_folds=5, w_grid=NULL)
    beta_temp = stv.fit[["beta"]]
    
    beta1_temp = beta_temp[ ,1]
    beta2_temp = beta_temp[ ,2]
    beta3_temp = beta_temp[ ,3]
    
    result1_work_temp = cbind(l, beta1_temp, w_work)
    result2_work_temp = cbind(l, beta2_temp, w_work)
    result3_work_temp = cbind(l, beta3_temp, w_work)
    
    result1_work = rbind(result1_work, result1_work_temp)
    result2_work = rbind(result2_work, result2_work_temp)
    result3_work = rbind(result3_work, result3_work_temp)
    
    res_conv_work_temp = stv.fit$conv
    res_conv_work = c(res_conv_work, res_conv_work_temp)
  }
  
  return(list(result1 = result1_work, result2 = result2_work, result3 = result3_work, res_conv = res_conv_work))
}

### find turning points

get_w1_idx_left = function(num, df) {
  rownames(df) = c(1:num)
  colnames(df) = c("i", "beta", "w")
  
  sign_df = sign(df$beta)
  
  for (i in 2:(num-1)) {
    test = sign_df[i]
    test_left = sign_df[i-1]
    test_right = sign_df[i+1]
    
    if (test_left*test_right == 0) {
      w1_idx = i+1
      return(w1_idx)
    } else if (test_left*test_right == -1) {
...
get_w1_idx_right = function(num, df) {
  rownames(df) = c(1:num)
  colnames(df) = c("i", "beta", "w")
  ...
  for (i in 2:(num-1)) {
    test_i = num + 1 - i
    test = sign_df[test_i]
    test_left = sign_df[test_i-1]
    test_right = sign_df[test_i+1]
...
### local FDR & percentile-t

calc_w1 = function(prep_data, q_left, q_right, n_btsp, n) {
  library(locfdr)
  
  prep_data = as.data.frame(prep_data)
  res_btsp_temp = matrix(ncol = 2, nrow = 0)
  
  for (l in 1:n_btsp) {
    wd = prep_data[which(prep_data$l == l), ]
    wd = cbind.data.frame(l = wd$l, beta = wd$beta, w = wd$w)
    
    w1_index_left = get_w1_idx_left(num = n, df = wd)
    if (w1_index_left < n) {
      w1_left = wd$w[w1_index_left]
    } else {
      w1_left = NA
    }
    
    w1_index_right = get_w1_idx_right(num = n, df = wd)
    if (w1_index_right > 1) {
      w1_right = wd$w[w1_index_right]
    } else {
      w1_right = NA
    }
    
    res = c(l, w1_left, w1_right)
    res_btsp_temp = rbind.data.frame(res_btsp_temp, res)
    
  }
  
  colnames(res_btsp_temp) = c("dataset_idx", "w1_left", "w1_right")
  
  # for w1_left
  work_w1_left = res_btsp_temp$w1_left[!is.na(res_btsp_temp$w1_left)]
  
  result_left_temp = 
    tryCatch(
      {
        res_left_prep = locfdr(
          work_w1_left,
          pct0 = c(q_left, q_right),
          plot = 0
        )
        res_mean_left = res_left_prep$fp0[3,1]
        res_SD_left = res_left_prep$fp0[3,2]
        
        res_mu_left_prep = work_w1_left[which(res_left_prep$fdr>=0.1)]
        res_mu_left = as.numeric(scale(res_mu_left_prep))
        res_mu_left_0.025 = quantile(res_mu_left, probs = 0.025)
        res_mu_left_0.975 = quantile(res_mu_left, probs = 0.975)
        
        result_left_temp = c(res_mean_left, res_SD_left, res_mu_left_0.025, res_mu_left_0.975)
      },
      error = function(e) {
        res_mean_left = mean(work_w1_left)
        res_SD_left = sd(work_w1_left)
        
        res_mu_left = as.numeric(scale(work_w1_left))
        res_mu_left_0.025 = quantile(res_mu_left, probs = 0.025)
        res_mu_left_0.975 = quantile(res_mu_left, probs = 0.975)
        
        return(c(res_mean_left, res_SD_left, res_mu_left_0.025, res_mu_left_0.975))
      }
    )

  Result_left = result_left_temp
  
  # for w1_right
  work_w1_right = res_btsp_temp$w1_right[!is.na(res_btsp_temp$w1_right)]
  
  result_right_temp = 
    tryCatch(
      {
        res_right_prep = locfdr(
          work_w1_right,
          pct0 = c(q_left, q_right),
          plot = 0
        )
        res_mean_right = res_right_prep$fp0[3,1]
        res_SD_right = res_right_prep$fp0[3,2]
        
        res_mu_right_prep = work_w1_right[which(res_right_prep$fdr>=0.1)]
        res_mu_right = as.numeric(scale(res_mu_right_prep))
        res_mu_right_0.025 = quantile(res_mu_right, probs = 0.025)
        res_mu_right_0.975 = quantile(res_mu_right, probs = 0.975)
        
        result_right_temp = c(res_mean_right, res_SD_right, res_mu_right_0.025, res_mu_right_0.975)
      },
      error = function(e) {
        res_mean_right = mean(work_w1_right)
        res_SD_right = sd(work_w1_right)
        
        res_mu_right = as.numeric(scale(work_w1_right))
        res_mu_right_0.025 = quantile(res_mu_right, probs = 0.025)
        res_mu_right_0.975 = quantile(res_mu_right, probs = 0.975)
        
        return(c(res_mean_right, res_SD_right, res_mu_right_0.025, res_mu_right_0.975))
      }
    )

  Result_right = result_right_temp
  
  # result
  Result_w1 = rbind.data.frame(Result_left, Result_right)
  colnames(Result_w1) = c("mean", "SD", "q_0.025", "q_0.975")
  rownames(Result_w1) = c("left", "right")
  
  return(Result_w1)
}

### bootstrap-based confidence intervals and coverage probability

calc_btspCI = function(wd_ori, wd_btsp, true_left, true_right) {
  
  # wd_ori should be organized as:
  # col 1: dataset index
  # col 2: estimated value for left turning point
  # col 3: estimated value for right turning point
  
  colnames(wd_ori) = c("idx", "w1_left", "w1_right")
  
  idx_zero = which(is.na(wd_ori$w1_left) * is.na(wd_ori$w1_right) == 1)
  objID_nozero = wd_ori[-idx_zero,]$idx
  
  # wd_btsp should be organized as:
  # col 1: dataset index, should have the same order as that in wd_ori
  # col 2-5: mean, SD, 0.025 quantile, 0.975 quantile for left turning point
  # col 6-9: mean, SD, 0.025 quantile, 0.975 quantile for right turning point
  
  colnames(wd_btsp) = c("obj", 
                        "Mean_left", "SD_left", "Mu_left_0.025", "Mu_left_0.975",
                        "Mean_right", "SD_right", "Mu_right_0.025", "Mu_right_0.975")
  res_btsp = wd_btsp[which(wd_btsp$obj %in% objID_nozero), ]
  
  # coverage probability
  # cp left
  bias_left = res_btsp$Mean_left - wd_ori[-idx_zero,]$w1_left
  CI_lower_left = res_btsp$Mean_left - res_btsp$SD_left * res_btsp$Mu_left_0.975 - bias_left
  CI_upper_left = res_btsp$Mean_left - res_btsp$SD_left * res_btsp$Mu_left_0.025 - bias_left
  cp_left = mean(ifelse(CI_lower_left<true_left, 1, 0) * ifelse(CI_upper_left>true_left, 1, 0), na.rm = TRUE)
  
  # "average" CI left
  CI_lower_left_avgbtsp=mean(CI_lower_left)
  CI_upper_left_avgbtsp=mean(CI_upper_left)
  
  # cp right
  bias_right = res_btsp$Mean_right - wd_ori[-idx_zero,]$w1_right
  CI_lower_right = res_btsp$Mean_right - res_btsp$SD_right * res_btsp$Mu_right_0.975 - bias_right
  CI_upper_right = res_btsp$Mean_right - res_btsp$SD_right * res_btsp$Mu_right_0.025 - bias_right
  cp_right = mean(ifelse(CI_lower_right<true_right,1,0) * ifelse(CI_upper_right>true_right,1,0), na.rm = TRUE)
  
  # "average" CI right
  CI_lower_right_avgbtsp=mean(CI_lower_right)
  CI_upper_right_avgbtsp=mean(CI_upper_right)
  
  # result
  res_left = c(cp_left, CI_lower_left_avgbtsp, CI_upper_left_avgbtsp)
  res_right = c(cp_right, CI_lower_right_avgbtsp, CI_upper_right_avgbtsp)
  res = rbind.data.frame(res_left, res_right)
  colnames(res) = c("coverage probability", "95% CI, lower", "95% CI, upper")
  rownames(res) = c("left", "right")
